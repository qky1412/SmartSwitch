...
function initData() {
    JDSMART.io.initDeviceData(
        function (suc) {
            ...
            refreshOutputDeviceData();
        });
}
//刷新输出设备各个插槽数据
function refreshOutputDeviceData() {
    ...
    //    中间值对应不同的灰度（亮度）。
    CloudApi.queryOutputDevStatus(function(result, data) {
        alert("查询输出设备状态" + JSON.stringify(data));
        if(result) {
            alert("查询输出设备状态成功" + JSON.stringify(data));
        }else {
            alert("查询输出设备状态失败");
        }
    });
